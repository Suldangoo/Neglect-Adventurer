...
public class BackendPostSystem : MonoBehaviour
{
	...
	public void PostListGet(PostType postType)
	{
		...
		try
		{
			LitJson.JsonData jsonData = bro.GetFlattenJSON()["postList"];
	...
	public void PostReceive(PostType postType, int index)
	{
		...
		var bro = Backend.UPost.ReceivePostItem(postType, postList[index].inDate);

		if (!bro.IsSuccess())
		{
			Debug.LogError($"{postType.ToString()}의 {postList[index].inDate} 우편수령 중 에러가 발생했습니다. : {bro}");
			return;
		}

		Debug.Log($"{postType.ToString()}의 {postList[index].inDate} 우편수령에 성공했습니다. : {bro}");
	...
	public void PostReceiveAll(PostType postType)
	{
		...
		if (!bro.IsSuccess())
		{
			Debug.LogError($"{postType.ToString()} 우편 전체 수령 중 에러가 발생했습니다. : {bro}");
			return;
		}

		Debug.Log($"우편 전체 수령에 성공했습니다. : {bro}");
		...
		try
		{
			foreach ( LitJson.JsonData itemJson in item )
			{
				...
				if ( chartName.Equals(Constants.GOODS_CHART_NAME) )
				{
					...
					{
						BackendGameData.Instance.UserGameData.diamond += itemCount;
					}
				}
